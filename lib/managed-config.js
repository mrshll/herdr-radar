...
// 'terminal', which has no hex), `variant` its side, and `mode` the
// `[theme.custom.<side>]` table Herdr layers on top, set only under its own
// `auto_switch`.
//
// With appearance following on, applyAppearance drives the name, so the
// desktop's side answers it. `auto_switch` left to Herdr picks by asking the
...
function effectiveTheme(
  ...
  const bySide = (side) => ({
    theme:
      palette.canonicalTheme(tableValue(text, 'theme', side === 'light' ? 'light_name' : 'dark_name')) ??
      (side === 'light' ? 'catppuccin-latte' : 'catppuccin'),
    variant: side,
  });
  if (detected) return { ...bySide(detected), mode: null };
  if (tableRaw(text, 'theme', 'auto_switch') === 'true') {
    const side = host() ?? 'dark';
    return { ...bySide(side), mode: side };
  }
  const theme = palette.canonicalTheme(tableValue(text, 'theme', 'name')) ?? 'catppuccin';
  if (theme === 'terminal') return { theme, variant: host() ?? 'dark', mode: null };
  return { theme, variant: palette.lightThemes.includes(theme) ? 'light' : 'dark', mode: null };
}
