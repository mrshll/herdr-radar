...
// The desktop's side, when it drives `[theme] name` — appearance following on
// and a `[theme]` table for applyAppearance to rewrite. Without that table
// applyAppearance refuses, so Herdr keeps drawing whatever the file says, and
// a light desktop must not put light rows and a light row fill over it.
function drivenSide(text, detected = config.followAppearance ? appearance.current() : null) {
  return detected && /^\[theme\]\s*$/m.test(text) ? detected : null;
}

// Which side of the palette to use: light or dark, or null for "leave it".
function chromeVariant(text, detected = config.followAppearance ? appearance.current() : null) {
  ...
  // name is the only input — unless `auto_switch` is on, where Herdr ignores
  // the name and neither row fill would be right half the time.
  const driven = drivenSide(text, detected);
  if (driven) return driven;
  if (tableRaw(text, 'theme', 'auto_switch') === 'true') return null;
  const name = palette.canonicalTheme(tableValue(text, 'theme', 'name'));
  if (!name || name === 'terminal') return null;
  return palette.lightThemes.includes(name) ? 'light' : 'dark';
}

// The theme Herdr draws for one side under its own `auto_switch` (src/app/mod.rs
// `theme_runtime_config`): the side's name if set, else the sibling of `name`
// (`sibling_theme_names` — tokyo-night pairs with tokyo-night-day, an unpaired
// theme such as nord is its own sibling), and an unknown name falls back to
// catppuccin for that side.
function autoSwitchTheme(text, side) {
  const fallback = side === 'light' ? 'catppuccin-latte' : 'catppuccin';
  const named = tableValue(text, 'theme', side === 'light' ? 'light_name' : 'dark_name');
  if (named) return palette.canonicalTheme(named) ?? fallback;
  const manual = palette.canonicalTheme(tableValue(text, 'theme', 'name') ?? 'catppuccin');
  if (!manual) return fallback;
  const pair = palette.themeSiblings.find((sibling) => sibling.includes(manual));
  return pair ? pair[side === 'light' ? 1 : 0] : manual;
}

// The theme Herdr will draw the sidebar in, resolved the way Herdr resolves it
// (src/app/mod.rs `resolve_effective_theme`): `theme` is canonical (or
// 'terminal', which has no hex), `variant` the side of the palette that suits
// it, and `mode` the `[theme.custom.<side>]` table Herdr layers on top, set
// only under its own `auto_switch`.
//
// With appearance following on, applyAppearance writes the side's name (or
// catppuccin's for that side) into `name`, and Herdr draws that name — an
// unknown one as catppuccin. `auto_switch` left to Herdr picks by asking the
// host terminal at attach time, which nothing here can see; the desktop is the
// best stand-in, and failing that Herdr's own assumption when the host does
// not answer, dark. `terminal` follows the host too, so it takes the same
// guess. A missing or unknown `name` is Herdr's default, catppuccin.
function effectiveTheme(
  text,
  detected = config.followAppearance ? appearance.current() : null,
  host = () => appearance.current(),
) {
  const sideOf = (theme, guess) =>
    theme === 'terminal' ? guess() : palette.lightThemes.includes(theme) ? 'light' : 'dark';
  const driven = drivenSide(text, detected);
  if (driven) {
    const named = tableValue(text, 'theme', driven === 'light' ? 'light_name' : 'dark_name');
    const theme = named
      ? (palette.canonicalTheme(named) ?? 'catppuccin')
      : driven === 'light'
        ? 'catppuccin-latte'
        : 'catppuccin';
    return { theme, variant: sideOf(theme, () => driven), mode: null };
  }
  if (tableRaw(text, 'theme', 'auto_switch') === 'true') {
    const side = host() ?? 'dark';
    const theme = autoSwitchTheme(text, side);
    return { theme, variant: sideOf(theme, () => side), mode: side };
  }
  const theme = palette.canonicalTheme(tableValue(text, 'theme', 'name')) ?? 'catppuccin';
  return { theme, variant: sideOf(theme, () => host() ?? 'dark'), mode: null };
}

// A Herdr theme colour as the `#rrggbb` a sidebar cell takes, or null. Herdr
// also accepts ANSI names and `reset` there, which are the host terminal's
// colours and have no hex to write.
function themeHex(value) {
  ...
  const rgb = color
    ...
    ?.slice(1)
    .map(Number);
  if (rgb && rgb.every((channel) => channel <= 255)) {
    return `#${rgb.map((channel) => channel.toString(16).padStart(2, '0')).join('')}`;
  }
  return null;
}

// The theme's `text` and `subtext0` for the idle titles, with any the user set
// under `[theme.custom]` (then the active side's table, as Herdr layers them)
// winning — they are what Herdr's own text is drawn in.
function sidebarInk(text, { theme, variant, mode }) {
  const ink = { ...palette.inkForTheme(theme, variant) };
  const tables = mode ? ['theme.custom', `theme.custom.${mode}`] : ['theme.custom'];
  for (const table of tables) {
    for (const key of ['text', 'subtext0']) {
      const custom = themeHex(tableValue(text, table, key));
      if (custom) ink[key] = custom;
    }
  }
  return ink;
}

// The whole sidebar block for what `text` configures.
function sidebarBlockFor(text, detected) {
  const effective = effectiveTheme(text, detected);
  return sidebarBlock(effective.variant, sidebarInk(text, effective));
}
...
function applyAppearance(variant) {
  ...
  next = foreignTheme
    ? dropBlock(next, THEME_START, THEME_END)
    : withThemeBlock(next, variant, effectiveTheme(next, variant).theme);
  ...
  else if (next.includes(SIDEBAR_START))
    next = upsertTail(next, SIDEBAR_START, SIDEBAR_END, sidebarBlockFor(next, variant));
...
// The chrome tokens for one appearance: the theme's own selection fill
// (palette.rowFillFor), with the user's `[colors]` overrides applied. An empty
// override drops the token, so the theme's own `active_row_bg` shows through.
function chromeTokens(variant, theme) {
  const tokens = { active_row_bg: palette.rowFillFor(theme, variant) };
...
function themeBlock(variant, theme) {
  const tokens = chromeTokens(variant, theme);
...
function withThemeBlock(text, variant, theme) {
  const body = themeBlock(variant, theme);
  return body ? upsertTail(text, THEME_START, THEME_END, body) : dropBlock(text, THEME_START, THEME_END);
}

// The theme block for what `text` configures, or null where the fill is left
// to Herdr: no `[theme]`, `auto_switch`, `terminal`.
function themeBlockFor(text, detected = config.followAppearance ? appearance.current() : null) {
  const variant = chromeVariant(text, detected);
  return variant ? themeBlock(variant, effectiveTheme(text, detected).theme) : null;
}
...
function sidebarBlock(variant, themeInk = palette.inkForTheme(null, variant)) {
  const brand = palette.brand;
  const ink = palette.inkFor(variant);
  const state = palette.stateFor(variant, themeInk);
  ...
  const agentRow = (vendor) =>
    [
      ...
      `[${[
        ...
        // Not dimmed. Terminal dim blends toward the background, and on top
        // of a colour that already recedes it took a stale title below 2:1 —
        // receded past reading. The theme's second-rank ink is the step down;
        // the stale logo beside it still fades.
...
function apply() {
  ...
  const theme = themeBlockFor(report.text);
  next = theme ? upsertTail(next, THEME_START, THEME_END, theme) : dropBlock(next, THEME_START, THEME_END);
  next = upsertTail(next, SIDEBAR_START, SIDEBAR_END, sidebarBlockFor(report.text));
  writeAtomic(report.file, next, identity.TMP_SUFFIX);
...
function setSidebarRows(on) {
  ...
  const next = on
    ? upsertTail(report.text, SIDEBAR_START, SIDEBAR_END, sidebarBlockFor(report.text))
