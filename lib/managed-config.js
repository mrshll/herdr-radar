...
// The built-in Herdr will draw the sidebar over, canonical, or null when the
// file cannot say. No `[theme] name` is Herdr's own default, catppuccin. With
// appearance following on, applyAppearance drives the name, so the side the
// desktop is on answers it; `auto_switch` left to Herdr picks by asking the
// host terminal at attach time, which nothing here can see.
function sidebarTheme(text, detected = config.followAppearance ? appearance.current() : null) {
  if (detected) {
    const side = tableValue(text, 'theme', detected === 'light' ? 'light_name' : 'dark_name');
    return palette.canonicalTheme(side ?? (detected === 'light' ? 'catppuccin-latte' : 'catppuccin'));
  }
  if (tableRaw(text, 'theme', 'auto_switch') === 'true') return null;
  return palette.canonicalTheme(tableValue(text, 'theme', 'name') ?? 'catppuccin');
}

// Which side of the palette the sidebar rows take: the chrome's side when we
// drive the chrome, else the side of the theme the file selects. The bare
// 'light' fallback used to apply to a config with no `[theme]` at all, which
// Herdr draws in catppuccin — the light palette's greys on a dark panel.
function sidebarVariant(text, theme) {
  const variant = chromeVariant(text);
  if (variant) return variant;
  if (!theme || theme === 'terminal') return 'light';
  return palette.lightThemes.includes(theme) ? 'light' : 'dark';
}

// The theme's `text` and `subtext0` for the idle titles, with any the user set
// under `[theme.custom]` winning — Herdr applies those on top of the built-in,
// so they are what its own text is drawn in.
function sidebarInk(text, variant, theme) {
  const ink = { ...palette.inkForTheme(theme, variant) };
  for (const key of ['text', 'subtext0']) {
    const custom = tableValue(text, 'theme.custom', key);
    if (/^#[0-9a-f]{6}$/i.test(custom ?? '')) ink[key] = custom.toLowerCase();
  }
  return ink;
}

// The whole sidebar block for what `text` configures.
function sidebarBlockFor(text) {
  const theme = sidebarTheme(text);
  const variant = sidebarVariant(text, theme);
  return sidebarBlock(variant, sidebarInk(text, variant, theme));
}
...
function applyAppearance(variant) {
  ...
  if (foreignSidebar) next = dropBlock(next, SIDEBAR_START, SIDEBAR_END);
    ...
    const theme = palette.canonicalTheme(wanted);
    next = upsertTail(next, SIDEBAR_START, SIDEBAR_END, sidebarBlock(variant, sidebarInk(next, variant, theme)));
  }
