...
module.exports = {
  ...
  // One kind of pane that outranks the rest: a supervisor, a coordinator — the
  // agent you talk to about all the others. `pin_token = "role=supervisor"`
  // matches panes whose metadata carries that token (any source); they sort
  // above every group in both orders, sit in a group of their own with no
  // workspace header, and lead with `pin_label` in the accent colour. Absent
  // means nothing is pinned.
  pin: parsePin(raw.pin_token),
  pinLabel: typeof raw.pin_label === 'string' && raw.pin_label ? raw.pin_label : 'supervisor',

  // Group by a metadata token instead of by workspace. An orchestrator that
  // seats helpers in several workspaces or worktrees tags each pane with the
  // task it belongs to (`group_token = "owner"`); panes sharing a value form
  // one top-level group headed by that value, whatever repo or worktree they
  // sit in. `group_lead = "role=coordinator,primary"` names the pane that leads
  // the group: it sorts first, and the rest hang under it. Panes without the
  // token keep the workspace and worktree grouping. Absent means off.
  groupToken: typeof raw.group_token === 'string' && raw.group_token ? raw.group_token : null,
  groupLead: parseLead(raw.group_lead),

  // A short label in front of a pane's title, read from a metadata token:
  // `tag_token = "role"` with `tag_labels = "orchestrator=orc,implementer=imp"`
  // leads a pane whose `role` is `orchestrator` with `[orc]`. A value missing
  // from `tag_labels` gets no label; without `tag_labels` the token's own value
  // is shown. A pinned pane never gets one — its pin mark already says what it
  // is. Absent means off.
  tagToken: typeof raw.tag_token === 'string' && raw.tag_token ? raw.tag_token : null,
  tagLabels: parseLabels(raw.tag_labels),
};
...
function parseLabels(spec) {
  ...
  for (const pair of spec.split(',')) {
    ...
    if (at > 0) labels.set(pair.slice(0, at).trim(), pair.slice(at + 1).trim());
  }
  return labels;
}
...
function parsePin(spec) {
  ...
  if (at <= 0) return null;
  return { name: spec.slice(0, at).trim(), value: spec.slice(at + 1).trim() };
}
