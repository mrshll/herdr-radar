...
// Every key a pinned pane publishes (config.pin). `~` sorts after every digit,
// so under a descending sort it lands above any real stamp in both orders.
const PINNED_KEY = '~';
// The group a pane is drawn and ranked in. Normally its workspace; a pinned
// pane is a group of its own, and a pane tagged with a task (config.groupToken)
// joins that task's group wherever it sits. The group id stands in for the
// workspace everywhere the panel's grouping is worked out; the Spaces marks
// keep the real workspace.
const groupOf = (entry) =>
  entry.pinned ? `pin:${entry.pane}` : entry.owner ? `owner:${entry.owner}` : entry.workspace;
// A pane that hangs under its task's lead rather than leading.
const isMember = (entry) => !entry.pinned && Boolean(entry.owner) && !entry.lead;
...
class Frame {
  ...
  async render(now) {
    ...
    // Pinned and task-tagged panes carry their group id (groupOf) in place of
    // their workspace here; everything that draws group furniture reads this
    // list, everything that aggregates by workspace reads `entries`.
    const displayEntries = this.displayOrder(entries, viewMode, keys).map((entry) =>
      groupOf(entry) === entry.workspace ? entry : { ...entry, workspace: groupOf(entry) },
    );
    // Headers: a task group is named by its task, a pinned pane by nothing (its
    // pin mark leads instead), everything else by its workspace.
    const groupLabels = new Map(workspaces);
    for (const entry of entries) {
      if (entry.owner && !entry.pinned) groupLabels.set(groupOf(entry), entry.owner);
    }
    ...
    // Likewise the last member of each task group, which closes its tree.
    const memberLast = new Map();
    if (grouped) {
      for (const entry of displayEntries) {
        if (keys.splitChild(entry)) splitLast.set(entry.tab, entry.pane);
        if (isMember(entry) && !heads.has(entry.pane)) memberLast.set(entry.workspace, entry.pane);
      }
    }
    ...
    // group -> the same, for fading a whole group's header when it goes stale.
    ...
    for (const entry of entries) {
      ...
      if (entry.workspace) {
        ...
        if (!groupAgents.has(group)) groupAgents.set(group, []);
        groupAgents.get(group).push({ display, name: entry.name });
      }
      ...
      const group = groupOf(entry);
      const depth = grouped && !entry.pinned ? (heads.has(entry.pane) ? 0 : 1) + (keys.nested(group) ? 1 : 0) : 0;
      ...
      // from, drawn the way a worktree hangs off its checkout; a task's
      // members hang off its lead the same way.
      const corner = !grouped
        ? ''
        : keys.splitChild(entry)
          ? splitLast.get(entry.tab) === entry.pane
            ? '└─ '
            : '├─ '
          : isMember(entry) && !heads.has(entry.pane)
            ? memberLast.get(group) === entry.pane
              ? '└─ '
              : '├─ '
            : '';
      // The header text this row will sit under, for the prefix trim. Empty
      // in the flat view: nothing is repeated there.
      const header = grouped && config.trimGroupPrefix ? (groupLabels.get(group) ?? '') : '';
    ...
    await this.groupJobs(entries, displayEntries, viewMode, grouped, groupAgents, groupLabels, keys, now, deadlines);
  ...
  sortKeys(entries, parents, worktrees) {
    ...
    // A pinned pane ranks by PINNED_KEY alone and takes no part in its
    // workspace's or its tab's ranking, so the rest of its workspace sorts as
    // if it were not there.
    const sortKeyFor = (entry) => (entry.pinned ? PINNED_KEY : minuteKey(entry.pane));
    // Keyed by group (groupOf), which is the workspace unless a pane is pinned
    // or tagged with a task.
    const ownKeys = new Map();
    for (const entry of entries) {
      if (!entry.workspace || entry.pinned) continue;
      const key = minuteKey(entry.pane) ?? NO_STAMP;
      const group = groupOf(entry);
      const best = ownKeys.get(group);
      if (best === undefined || key > best) ownKeys.set(group, key);
    }
    ...
    // Task groups are not workspaces, so they never appear in `parents` or
    // `worktrees` and stay top-level: a task is not a branch of anything.
    const present = new Set(entries.map(groupOf));
    ...
    for (const entry of entries) {
      if (!entry.tab || entry.pinned) continue;
    ...
    for (const entry of entries) {
      // A task's lead ranks first within its group, ahead of any member
      // however recently that member worked.
      if (entry.pinned || (entry.owner && entry.lead)) {
        tabKeys.set(entry.pane, PINNED_KEY);
        continue;
      }
    ...
    // pane and hangs off nothing. A pinned pane hangs off nothing either.
    const split = (entry) => !entry.pinned && Boolean(entry.tab) && (tabPanes.get(entry.tab)?.length ?? 0) > 1;
    const splitChild = (entry) => split(entry) && tabHead.get(entry.tab) !== entry.pane;
    const wsKeyFor = (entry) => (entry.pinned ? `${PINNED_KEY}-${entry.pane}` : (wsKeys.get(groupOf(entry)) ?? null));

    return { minuteKey, sortKeyFor, wsKeys, wsKeyFor, parentOf, orphanRepo, nested, tabKeys, splitChild };
  }
  ...
  displayOrder(entries, viewMode, { wsKeyFor, sortKeyFor, tabKeys }) {
  ...
  paneJobs(entry, display, { tabs, keys, indent, corner = '', spinStep, header = '' }, now, deadlines, jobs) {
    ...
    // The pane's tag (config.tagToken) leads the title, so it wears the same
    // state colour and fades with the row.
    const trimmed = state.trimGroupPrefix(entry.title, header);
    const title = entry.tag && !entry.pinned && trimmed ? `[${entry.tag}] ${trimmed}` : trimmed;
    ...
    const key = line
      ? `${display}:${line.pin}:${line.mark}:${line.split}:${line.logo}:${line.titlePrefix}:${title}`
      : '';
    ...
    const sortKey = keys.sortKeyFor(entry);
    const wsKey = entry.workspace ? keys.wsKeyFor(entry) : null;
