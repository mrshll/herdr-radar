...
// The pinned pane's lead (config.pin): `◆ <pin_label>` in the accent colour,
// standing in for the workspace header it does not get. Empty on every other
// pane, where the cell collapses.
const PIN_TOKEN = 'pin_mark';
...
function tagFor(tokens) {
  const value = config.tagToken ? tokens[config.tagToken] : undefined;
  if (typeof value !== 'string' || !value) return '';
  if (!config.tagLabels) return value;
  return config.tagLabels.get(value) ?? '';
}
    ...
    return [
      {
        ...
        // Matched against every source's tokens, not just ours: the pin is
        // declared by whoever seats the pane (config.pin).
        pinned: Boolean(config.pin) && tokens[config.pin.name] === config.pin.value,
        // The task this pane belongs to (config.groupToken), and whether it
        // leads that task's group. Empty when grouping by token is off.
        owner: config.groupToken && typeof tokens[config.groupToken] === 'string' ? tokens[config.groupToken] : '',
        lead: Boolean(config.groupLead) && config.groupLead.values.includes(tokens[config.groupLead.name]),
        // The short label that leads the title (config.tagToken); empty when
        // tagging is off or the pane carries no labelled value.
        tag: tagFor(tokens),
        tab: a.tab_id ?? '',
...
function composeLine(entry, display, tabLabel, indent, step = 0, corner = '') {
  ...
  return {
    pin: entry.pinned ? `◆ ${config.pinLabel}` : '',
    mark: mark.join(' '),
...
function stateTokens(display, line, title) {
  ...
  tokens[PIN_TOKEN] = line.pin || null;
  if (line.logo) {
...
function clearState(source, pane) {
  const tokens = { sort_key: null, ws_key: null, tab_key: null, [SPLIT_TOKEN]: null, [PIN_TOKEN]: null };
  for (const name of LOGO_TOKENS) tokens[name] = null;
  for (const state of STATES) {
    tokens[`name_${state}`] = null;
    ...
    entries.map((entry) => {
      ...
      const branch = parent
        ...
        : orphan
          ? `└─ ${mark}`
          : '';
      // A pinned pane is a group of its own whose header is its pin mark
      // (composeLine), so it takes no workspace name here.
      const name = heads.has(entry.pane) && !entry.pinned ? (wsLabels.get(entry.workspace) ?? entry.workspace) : null;
